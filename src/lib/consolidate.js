...
/**
 * ReadPartials `path` with `options` with
 * callback `(err, str)`. When `options.cache`
 * is true the partial strings will be cached.
 *
 * @param {String} path
...
function readPartials(path, options, fn) {
  ...
  function read(index) {
    ...
    if (index == keys.length) {
      return fn(null, partials);
    }

    var key = keys[index],
        fpath = join(dirname(path), partials[key]+extname(path)),
        str = cache[fpath];
    ...
    if (options.cache && str && typeof str === 'string') {
      ...
      fs.readFile(fpath, 'utf8', function(err, str){
        if (err) {
          ...
          if (options.cache) cache[fpath] = str;
          partials[key] = str;
          return read(++index);
        }
      });
    ...
    readPartials(path, options, function (err, partials) {
      ...
      try {
        for (partial in partials) {
          engine.registerPartial(partial, partials[partial]);
        }
        options.filename = path;
        var tmpl = engine.compile(str, options);
        fn(null, tmpl(options));
...
exports.mustache = function(path, options, fn) {
  ...
  read(path, options, function(err, str){
    ...
    readPartials(path, options, function (err, partials) {
      ...
      try {
        options.filename = path;
        fn(null, engine.to_html(str, options, partials));
      } catch (err) {
        fn(err);
      }
    });
